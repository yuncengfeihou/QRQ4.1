...
export function handleOutsideClick(event) {
    ...
    // Close whitelist panel if click is outside the panel and its trigger button
...
export function setupEventListeners() {
    ...
    if (!rocketButton) {
        console.error(`[${Constants.EXTENSION_NAME}] Rocket button not found. Cannot set up listeners.`);
        return;
    }
    ...
    safeAddListener(Constants.ID_MENU_STYLE_BUTTON, 'click', handleMenuStyleButtonClick);
    safeAddListener(`${Constants.ID_MENU_STYLE_PANEL}-close`, 'click', closeMenuStylePanel);
    safeAddListener(`${Constants.ID_MENU_STYLE_PANEL}-apply`, 'click', applyMenuStyles);
    safeAddListener(Constants.ID_RESET_STYLE_BUTTON, 'click', resetMenuStyles);
